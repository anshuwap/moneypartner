    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="card-tools">
          ...
          <a href="{{ url('admin/employee') }}" class="btn btn-sm btn-warning"><i class="fas fa-arrow-alt-circle-left"></i>&nbsp;Back</a>
        </div>
      </div>
      ...
      <div class="card-body table-responsive py-4">
        <table id="table" class="table table-hover table-sm text-nowrap">
          ...
          <tbody>
            ...
            <tr>
              <td>{{ ++$i }}</td>
              <td>{!! (!empty($comm['from_amount']))?mSign($comm['from_amount']):mSign(0) !!}</td>
              <td>{!! (!empty($comm['to_amount']))?mSign($comm['to_amount']):mSign(0) !!}</td>
              <td>{{ (!empty($comm['type']))?ucwords($comm['type']):'' }}</td>
              <td>{{ (!empty($comm['charges']))?$comm['charges']:0 }}</td>
              <td>
                @if (!empty($comm['status']) && $comm['status'] == 1)
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" id="bank_charges">
        ...
        <form id="add_bank_charges" action="{{ url('admin/employee-add-commission') }}" method="post">
          @csrf
...
<script>
  $('#add_bank_charges').click(function(e) {
    ...
    let url = '{{ url("admin/employee-add-commission") }}';
    $('#heading_bank').html('Add Commission');
    $('#put').html('');
  ...
  $(document).on('click', '.edit_bank_account', function(e) {
    ...
    var url = "{{ url('admin/employee-edit-commission') }}/" + id;
    $.ajax({
      url: url,
      method: 'GET',
      dataType: "JSON",
      ...
      success: function(res) {
        ...
        let urlU = '{{ url("admin/employee-update-commission") }}';
        $('#heading_bank').html('Edit Commision');
  ...
  $(document).on('click', '.activeVer', function() {
    ...
    $.ajax({
      'url': "{{ url('admin/employee-commission-status') }}/" + id + "/" + key + "/" + val,
      data: {},
      type: 'GET',
      dataType: 'json',
      success: function(res) {
