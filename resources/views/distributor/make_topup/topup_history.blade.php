        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="card-tools">
                    ...
                    <a href="{{ url('distributor/topup-history-export') }}{{ !empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''}}" class="btn btn-sm btn-success"><i class="fas fa-cloud-download-alt"></i>&nbsp;Export</a>
                    @if(!empty($filter))
                    ...
                    <!-- <a href="{{ url('distributor/make-topup') }}" class="btn btn-sm btn-warning mr-4" id=""><i class="fas fa-arrow-alt-circle-left"></i>&nbsp;Back</a> -->
                </div>
            </div>

            <div class="row pl-2 pr-2" id="filter" <?= (empty($filter)) ? "style='display:none'" : "" ?>>
                <div class="col-md-12 ml-auto">
                    <form action="{{ url('distributor/topup-history') }}">
                        <div class="form-row">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('click', '.activeVer', function() {
            ...
            $.ajax({
                'url': "{{ url('distributor/topup-status') }}",
                data: {
                    "_token": "{{ csrf_token() }}",
                    'id': id,
                    'status': val
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="add_topup_id" action="{{ url('distributor/make-topup') }}" method="post">
                    @csrf
                    <div id="put"></div>
                    <div class="row">
                        <div class="col-md-12">
...
<script>
    $('#payment_mode').change(function() {
        ...
        $.ajax({
            url: "{{ url('distributor/outlet-payment-mode') }}",
            data: {
    ...
    $('#payment_reference').change(function() {
        ...
        $.ajax({
            url: "{{ url('distributor/payment-details') }}",
            data: {
                'payment_mode': payment_mode,
    ...
    $('#create_topup').click(function(e) {
        ...
        let url = '{{ url("distributor/make-topup") }}';
    ...
    $(document).on('click', '.edit_topup_id', function(e) {
        ...
        var url = "{{ url('distributor/make-topup') }}/" + id + "/edit";
        $.ajax({
            url: url,
            method: 'GET',
            dataType: "JSON",
            ...
            success: function(res) {
                ...
                let urlU = '{{ url("distributor/make-topup") }}/' + id;
