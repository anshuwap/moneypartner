...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('click', '.remove_paymnet_channel', function() {
            ...
            var url = "{{ url('employee/payment-channel') }}/" + id;
        ...
        $('#table').DataTable({
            ...
            'ajax': {
                "dataType": "json",
                url: "{{ url('employee/payment-channel-ajax') }}",
                data: {}
            },
            columns: [{
                    data: "sl_no"
        ...
        $(document).on('click', '.activeVer', function() {
            ...
            $.ajax({
                'url': "{{ url('employee/payment-channel-status') }}",
                data: {
                    "_token": "{{ csrf_token() }}",
                    'id': id,
                    'status': val
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body" id="upi-id">
                <form id="add_payment_channel" action="{{ url('employee/payment-channel') }}" method="post">
                    @csrf
                    <div id="put"></div>
                    <div class="row">
                        <div class="col-md-12">
...
<script>
    $('#create_payment_channel').click(function(e) {
        ...
        let url = '{{ url("employee/payment-channel") }}';
    ...
    $(document).on('click', '.edit_paymnet_channel', function(e) {
        e.preventDefault();
        var id = $(this).attr('paymnet_channel_id');
        var url = "{{ url('employee/payment-channel') }}/" + id + "/edit";
        $.ajax({
            url: url,
            method: 'GET',
            dataType: "JSON",
            ...
            success: function(res) {
                ...
                let urlU = '{{ url("employee/payment-channel") }}/' + id;
