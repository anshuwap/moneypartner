...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('click', '.remove_comment', function() {
            ...
            var url = "{{ url('employee/comment') }}/" + id;
        ...
        $(document).on('click', '.activeVer', function() {
            ...
            $.ajax({
                'url': "{{ url('employee/comment-status') }}",
                data: {
                    "_token": "{{ csrf_token() }}",
                    'id': id,
                    'status': val
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body" id="upi-id">
                <form id="add_comment" action="{{ url('employee/comment') }}" method="post">
                    @csrf
                    <div id="put"></div>
                    <div class="row">
                        <div class="col-md-12">
...
<script>
    ...
    $(document).on('click', '.edit_comment', function(e) {
        e.preventDefault();
        var id = $(this).attr('comment_id');
        var url = "{{ url('employee/comment') }}/" + id + "/edit";
        $.ajax({
            url: url,
            method: 'GET',
            dataType: "JSON",
            ...
            success: function(res) {
                ...
                let urlU = '{{ url("employee/comment") }}/' + id;
