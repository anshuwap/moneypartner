...
<div class="row">
    ...
    <div class="col-md-8">
        ...
        <div class="card-body table-responsive py-2 table-sm">
            <div class="float-right">
                <a href="{{ url('employee/credit-export') }}{{ !empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''}}" class="btn btn-sm btn-success mr-2"><i class="fas fa-cloud-download-alt"></i>&nbsp;Export</a>
                @if(!empty($filter))
            ...
            <div class="row pl-2 pr-2" id="filter" <?= (empty($filter)) ? "style='display:none'" : "" ?>>
                <div class="col-md-12 ml-auto">
                    <form action="{{ url('employee/credit') }}">
                        <div class="form-row">
                            ...
                            <div class="form-group mt-4">
                                ...
                                <a href="{{ url('employee/credit') }}" class="btn btn-danger btn-sm"><i class="fas fa-eraser"></i>&nbsp;Clear</a>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <div class="col-md-4 mt-1">
        ...
        <div id="employee">
            <form id="add-employee" action="{{ url('employee/credit') }}" method="post" enctype="multipart/form-data">
                @csrf
...
<script>
    $('.switch-btn').change(function() {
        ...
        $.ajax({
            url: '{{ url("employee/credit-paid-status") }}',
            type: 'POST',
            data: {
                'id': id,
                'status': status
    ...
    $('#outlet_id').change(function() {
        ...
        $.ajax({
            url: '{{ url("employee/credit-show") }}/' + oultlet_id,
            type: "GET",
            dataType: "json",
            success: function(res) {
