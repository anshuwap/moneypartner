...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('click', '.remove_qr_code', function() {
            ...
            var url = "{{ url('employee/qr-code') }}/" + id;
        ...
        $(document).on('click', '.activeVer', function() {
            ...
            $.ajax({
                'url': "{{ url('employee/qr-code-status') }}",
                data: {
                    "_token": "{{ csrf_token() }}",
                    'id': id,
                    'status': val
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" id="allocate_retailer">
        <form id="allocate_retailer_account" action="{{ url('employee/q-save-allocate-retailer') }}" method="post">
          @csrf
          <div class="row">
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body" id="qr-code">
                <form id="add_qr_code" action="{{ url('employee/qr-code') }}" method="post">
                    @csrf
                    <div id="put"></div>
                    <div class="row">
                        <div class="col-md-12">
...
<script>
  $(document).on('click', '.allocate-retailer', function(e) {
    ...
    var url = "{{ url('employee/q-allocate-retailer') }}";
    $.ajax({
      url: url,
      method: 'GET',
      dataType: "JSON",
...
<script>
    ...
    $(document).on('click', '.edit_qr_code', function(e) {
        e.preventDefault();
        var id = $(this).attr('qr_code_id');
        var url = "{{ url('employee/qr-code') }}/" + id + "/edit";
        $.ajax({
            url: url,
            method: 'GET',
            dataType: "JSON",
            ...
            success: function(res) {
                ...
                let urlU = '{{ url("employee/qr-code") }}/' + id;
