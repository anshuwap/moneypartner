...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $(document).on('click', '.remove_bank_account', function() {
      ...
      var url = "{{ url('employee/bank-account') }}/" + id;
    ...
    $('#table').DataTable({
      ...
      'ajax': {
        "dataType": "json",
        url: "{{ url('employee/bank-account-ajax') }}",
        data: {}
      },
      columns: [{
          data: "sl_no"
    ...
    $(document).on('click', '.activeVer', function() {
      ...
      $.ajax({
        'url': "{{ url('employee/bank-account-status') }}",
        data: {
          "_token": "{{ csrf_token() }}",
          'id': id,
          'status': val
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" id="allocate_retailer">
        <form id="allocate_retailer_account" action="{{ url('employee/b-save-allocate-retailer') }}" method="post">
          @csrf
          <div class="row">
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" id="bank-account">
        <form id="add_bank_account" action="{{ url('employee/bank-account') }}" method="post">
          @csrf
          <div id="put"></div>
          <div class="row">
            <div class="col-md-12">
...
<script>
  $(document).on('click', '.allocate-retailer', function(e) {
    e.preventDefault();
    var id = $(this).attr('bank_account_id');
    var url = "{{ url('employee/b-allocate-retailer') }}";
    $.ajax({
      url: url,
      method: 'GET',
      dataType: "JSON",
...
<script>
  $('#create_bank_account').click(function(e) {
    ...
    let url = '{{ url("employee/bank-account") }}';
  ...
  $(document).on('click', '.edit_bank_account', function(e) {
    e.preventDefault();
    var id = $(this).attr('bank_account_id');
    var url = "{{ url('employee/bank-account') }}/" + id + "/edit";
    $.ajax({
      url: url,
      method: 'GET',
      dataType: "JSON",
      ...
      success: function(res) {
        ...
        let urlU = '{{ url("employee/bank-account") }}/' + id;
