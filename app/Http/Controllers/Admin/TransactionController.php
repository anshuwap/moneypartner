...
class TransactionController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if (!empty($start_date) && !empty($end_date)) {
                ...
                $start_date = strtotime(date('Y-m-d') . " 23:59:59");
            }
            ...
            $data['transaction'] = $query->orderBy('created', 'DESC')->paginate($perPage);
    ...
    public function bulkAction(Request $request)
    {
        ...
        foreach (explode(',', $request->trans_id) as $tran_id) {
            ...
            if (!$transaction->save())
                return response(['status' => 'error', 'msg' => 'Transaction Request not Success!']);

            if ($transaction->type == 'payout_api')
                webhook($transaction);
        }
        return response(['status' => 'success', 'msg' => 'Transaction Request Success!']);
    }
